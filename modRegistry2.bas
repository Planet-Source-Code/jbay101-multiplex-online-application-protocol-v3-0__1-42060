...
err_write:
    MsgBox "There was an error while writing (CreateNewKey) to the registry", 64
...
err_write:
    MsgBox "There was an error while writing (DeleteKey) to the registry", 64
...
err_write:
    MsgBox "There was an error while writing (DeleteValue) to the registry", 64
...
err_read:
    MsgBox "There was an error while reading (KeyCount) the registry", 64
...
err_read:
    MsgBox "There was an error while reading (KeyExists) the registry", 64

End Function

Function QueryValue(MainKey As Long, SubKey As String, ValueName As String, StringDwordBinary As String)

  Dim hKey As Long
  Dim vValue

    RegOpenKeyEx MainKey, SubKey, 0, KEY_ALL_ACCESS, hKey
    QueryValueEx hKey, ValueName, vValue, StringDwordBinary
    RegCloseKey (hKey)
    QueryValue = vValue

End Function

Private Function QueryValueEx(ByVal lhKey As Long, ByVal szValueName As String, vValue As Variant, StringBinaryDword As String) As Variant
  ...
  Dim sValue As String
    ...
    If StringBinaryDword = "Dword" Then lType = REG_DWORD
    If StringBinaryDword = "Binary" Then lType = REG_BINARY
    If StringBinaryDword = "String" Then lType = REG_SZ
...
QueryValueExError:
    MsgBox "There was an error while reading (QueryValue) the registry", 64
    Resume QueryValueExExit

End Function

Function SetKeyValue(MainKey As Long, SubKey As String, ValueName As String, ValueSetting As Variant, StringBinaryDword As String)
...
err_write:
    MsgBox "There was an error while writing (SetKeyValue) to the registry", 64
...
err_read:
    MsgBox "There was an error while reading (ValueCount) the registry", 64

End Function

':) Ulli's VB Code Formatter V2.13.6 (20.08.2002 00:17:33) 53 + 232 = 285 Lines
