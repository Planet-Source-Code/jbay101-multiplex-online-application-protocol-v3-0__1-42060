...
Function CheckCall(libName As String, funcName As String) As Boolean
CheckCall = True
Dim lb As Long, i As Integer
   ReDim mlngParameters(0)
   ReDim mbytCode(0)
   ...
   If lb = 0 Then
      CheckCall = False
      MsgBox "DLL: " & libName & " not found", vbCritical
      Exit Function
   End If
   mlngAddress = GetProcAddress(lb, ByVal funcName)
   If mlngAddress = 0 Then
        CheckCall = False
      MsgBox "Function " & funcName & " entry not found", vbCritical
      FreeLibrary lb
      Exit Function
   End If
