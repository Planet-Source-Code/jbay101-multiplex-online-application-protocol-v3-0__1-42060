...
Sock = HTTPRequest(URL, "GET", frmLoading.hwnd, GetProxy)
End Function

Function GetProxy() As String
Dim bolProxyEnabled As Boolean
Dim tmpBeg, tmpEnd

    strProxy = QueryValue(HKEY_CURRENT_USER, "Software\Microsoft\Windows\CurrentVersion\Internet Settings", "ProxyServer", "String")
    bolProxyEnabled = CBool(QueryValue(HKEY_CURRENT_USER, "Software\Microsoft\Windows\CurrentVersion\Internet Settings", "ProxyEnable", "Dword"))


    tmpBeg = InStr(1, strProxy, "http=", vbTextCompare)

    If tmpBeg > 0 And bolProxyEnabled Then
        tmpEnd = InStr(tmpBeg + 1, strProxy, ";")
        If tmpEnd = 0 Then tmpEnd = Len(strProxy)
        strProxy = Mid$(strProxy, tmpBeg + 5, tmpEnd - tmpBeg - 5)
    End If
    ...
    On Error Resume Next
        ...
        If Sock > 0 Then
            ...
            If bytes > 0 Then
                ...
                If Left$(ServerResponse, 7) = "HTTP/1." Then
                    'Skip server information
                    'ServerResponse = Mid$(ServerResponse, InStr(1, ServerResponse, vbCrLf & vbCrLf) + 4)
                End If

                DataArrival = DataArrival & ServerResponse
                Update = True
                frmLoading.Label1.caption = "Recieved " & Len(DataArrival) / 1024 & " KB / Unknown"
                frmLoading.Label1.Refresh
              ...
              ElseIf WSAGetLastError() <> WSAEWOULDBLOCK Then
                ...
                Sock = 0
                Update = False
                inf Len(DataArrival) / 1024 & " KB Downloaded, caching...", 0
            End If
        End If
...
Function DownloadAPP(URL As String, HDD As String)
frmLoading.Show
frmLoading.Refresh

inf "Caching application...", 0
frmLoading.Timer1.Interval = 250
frmLoading.Timer1.Enabled = True


GoTo h
HTTPRequest URL, "GET", frmLoading.hwnd, GetProxy

Dim stillgoing As Boolean
stillgoing = True

Do While stillgoing = True
    DoEvents
    stillgoing = Update
    DoEvents
Loop
...
If DownloadFromWeb(URL, HDD) < 0 Then
    MsgBox "Error downloading application." & vbCrLf & vbCrLf & "Either the file was unavailable or you are not connected to the internet.", vbCritical, "Fatal error"
    End
End If
